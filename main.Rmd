...
```{r}
pacman::p_load(tidyverse, dplyr, readxl, ggplot2)
```
...
mean_excess <- sapply(u_vals, function(u) mean(costos[costos > u] - u))
...
ggplot(df_umbral, aes(x = u_vals, y = mean_excess)) +
  ...
  theme_minimal()
```
## Función para encontrar el umbral para la cola de la distribución basado en la estabilidad de la función de exceso medio condicional
```{r}
